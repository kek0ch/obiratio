...
if(command === "say" && ['248521740945195008'].includes(message.author.id)){
...
if (message.content.startsWith(`${prefix}help`)){
    ...
    .setColor('RANDOM')
    .addField("Спиздить аву:", "`!avatar <Пользователь>`") 
    .addField("Реакции:", "`!kiss <Пользователь>;`\n`!slap <Пользователь>;`\n`!hug <Пользователь>;`\n`!kill <Пользователь>;`\n`!bite <Пользователь>;`\n`!lick <Пользователь>;`\n`!poke <Пользователь>;`\n`!pat <Пользователь>;`\n`!dance;`\n`!smug;`\n`!nom;`\n`!suicide;`\n`!cry;`\n`!happy.`")
    .addField("Тест на гея:", "`!gay <Пользователь>`")
    .addField("Совместимость:", "`!ship <Пользователь>`")
    .addField("Измерить пенис:", "`!penis <Пользователь>`")
    .addField("Призвать участника:", "`!summon <Пользователь>`")
    .addField("Оформить вкид:",  "`!vkid`")
    .addField("Прикурить калик:", "`!kalik`")
   .setTimestamp(); 
...
if (message.content.startsWith(`${prefix}slap`)) {
    if(!args[0]) return message.channel.send('```!slap @user\n\nПиздануть пользователя.```');
...
if (message.content.startsWith(`${prefix}kiss`)) {
    if(!args[0]) return message.channel.send('```!kiss @user\n\n Поцеловать пользователя.```');
    let user = message.author;
    let user1 = message.mentions.users.first();
    if (!user1 || user1.id === user.id) {
        user = bot.user;
...
if (message.content.startsWith(`${prefix}hug`)) {
    if(!args[0]) return message.channel.send('```!hug @user\n\nОбнять пользователя.```');
    let user = message.author;
    let user1 = message.mentions.users.first();
    if (!user1 || user1.id === user.id) {
        user = bot.user;
...
if (message.content.startsWith(`${prefix}bite`)) {
    if(!args[0]) return message.channel.send('```!bite @user\n\n Укусить участника.```');
    let user = message.author;
    let user1 = message.mentions.users.first();
    if (!user1 || user1.id === user.id) {
        user = bot.user;
...
if (message.content.startsWith(`${prefix}lick`)) {
    if(!args[0]) return message.channel.send('```!lick @user\n\n Облизать участника.```');
    let user = message.author;
    let user1 = message.mentions.users.first();
    if (!user1 || user1.id === user.id) {
        user = bot.user;
...
if (message.content.startsWith(`${prefix}poke`)) {
    if(!args[0]) return message.channel.send('```!poke @user\n\n Тыкать в участника.```');
    let user = message.author;
    let user1 = message.mentions.users.first();
    if (!user1 || user1.id === user.id) {
        user = bot.user;
...
if (message.content.startsWith(`${prefix}pat`)) {
    if(!args[0]) return message.channel.send('```!pat @user\n\nПогладить участника.```');
    let user = message.author;
    let user1 = message.mentions.users.first();
    if (!user1 || user1.id === user.id) {
        user = bot.user;
...
if (message.content.startsWith(`${prefix}suicide`)) {
    ...
    if (message.content.startsWith(`${prefix}kill`)) {
        if(!args[0]) return message.channel.send('```!kill @user\n\nУбить пользователя.```');
        let user = message.author;
        let user1 = message.mentions.users.first();
        if (!user1 || user1.id === user.id) {
            user = bot.user;
